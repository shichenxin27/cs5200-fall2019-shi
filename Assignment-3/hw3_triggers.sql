...
DELIMITER &&
CREATE TRIGGER cs5200_jdbc.website_privilege_insert_trigger AFTER INSERT ON cs5200_jdbc.website_role FOR EACH ROW
BEGIN
...
DELIMITER &&
CREATE TRIGGER cs5200_jdbc.website_privilege_update_trigger AFTER UPDATE ON cs5200_jdbc.website_role FOR EACH ROW
BEGIN
...
DELIMITER &&
CREATE TRIGGER cs5200_jdbc.website_privilege_delete_trigger AFTER DELETE ON cs5200_jdbc.website_role FOR EACH ROW
BEGIN
DELETE FROM website_privilege WHERE website = OLD.website AND developer = OLD.developer;
END;
&&
...
DELIMITER &&
CREATE TRIGGER cs5200_jdbc.page_privilege_insert_trigger AFTER INSERT ON cs5200_jdbc.page_role FOR EACH ROW
BEGIN
IF NEW.role = 'owner' OR NEW.role = 'admin' THEN
...
DELIMITER &&
CREATE TRIGGER cs5200_jdbc.page_privilege_update_trigger AFTER UPDATE ON cs5200_jdbc.page_role FOR EACH ROW
BEGIN
...
DELIMITER &&
CREATE TRIGGER cs5200_jdbc.page_privilege_delete_trigger AFTER DELETE ON cs5200_jdbc.page_role FOR EACH ROW
BEGIN
DELETE FROM page_privilege WHERE page = OLD.page AND developer = OLD.developer;
END
&&